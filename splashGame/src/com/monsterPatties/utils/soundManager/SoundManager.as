...
{		
	...
	public class SoundManager extends EventDispatcher
	{
		...
		private var _soundEffect2:Sound;
		private var _soundEffectChannel_1:SoundChannel;
		private var _soundEffectChannel_2:SoundChannel;
		private var _sfxSoundTransForm:SoundTransform;
		private var _sfxSoundTransForm2:SoundTransform;
		...
		private var _sfxVol:Number=0;
		private var _sfxVol2:Number=0;
		...
		private function prepareTransform():void 
		{
			...
			_sfxSoundTransForm2 = new SoundTransform();
		...
		public function increaseVolume():void 
		{
			...
			updateSfxSoundTransform();
		}
		
		private function updateSfxSoundTransform():void{
			if ( _soundEffectChannel_1 != null && _playingSfx <= 1 ) {
				_sfxSoundTransForm.volume = _sfxVol;
				_soundEffectChannel_1.soundTransform = _sfxSoundTransForm;
			}
			
			if ( _soundEffectChannel_2 != null && _playingSfx > 2 ) {
				_sfxSoundTransForm2.volume = _sfxVol;				
				_soundEffectChannel_2.soundTransform = _sfxSoundTransForm2;
			}
		}
		...
		{			
			if( !_sfxOff && _sfxVol > 0){
				_isSfxLoop = loop;
				var sfxSoundTransform:SoundTransform;
				if ( ( _playingSfx + 1 ) < 30 && _soundEffect != null ) {										
					try {
						if(_playingSfx <= 1 ){
							_playingSfx++;
							_sfxStillPlaying = true;
							_soundEffectChannel_1 = _soundEffect.play();
							sfxSoundTransform = new SoundTransform(_sfxVol);
							_soundEffectChannel_1.soundTransform = sfxSoundTransform;						
							_soundEffectChannel_1.addEventListener( Event.SOUND_COMPLETE, onSoundEffectComplete );
						}else{							
							_playingSfx++;
							_sfxStillPlaying = true;
							_soundEffectChannel_2 = _soundEffect.play();
							sfxSoundTransform = new SoundTransform(_sfxVol2);
							_soundEffectChannel_2.soundTransform = sfxSoundTransform;						
							_soundEffectChannel_2.addEventListener( Event.SOUND_COMPLETE, onSoundEffectComplete );
						}
					}catch (err:Error){
						trace( "soundManager Error: playsfx"  );
					}
					trace("number of playing sfx " + _playingSfx);					
				}
			}
		}
		...
		{	
			...
			if( _soundEffectChannel_2 != null && _soundEffect != null ){
				_soundEffectChannel_2.stop();				
			}
		}
		...
		public function removeSoundEffect():void 
		{
			if( _soundEffectChannel_1 != null && _soundEffect != null ){
				...
				_soundEffectChannel_2.removeEventListener( Event.SOUND_COMPLETE, onSoundEffectComplete );
				_soundEffect = null;
			}
		}		
		...
		{			
			...
			_sfxOff = true;
			if( _soundEffectChannel_1 != null && _soundEffectChannel_2 != null ){
				_soundEffectChannel_1.soundTransform = _sfxSoundTransForm;
				_soundEffectChannel_2.soundTransform = _sfxSoundTransForm2;
			}
		...
		public function loadSfx( id:String , sfx:*, vol:Number = 0.1 ):void 
		{
