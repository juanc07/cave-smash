package components.interactive 
{
	 ...
	 * @author jc
	 */
	public class ItemStandHammer extends StaticItemBase
	{
		
		private var isHitPlayer:Boolean = false;
		private var inGameHelp:InGameMessageMC;		
		
		public function ItemStandHammer() 
		{
			super();
		}
		...
		{
			if( !isHitPlayer &&  !gdc.hasComboAttack() ){
				isHitPlayer = true;
				inGameHelp = new InGameMessageMC();
				inGameHelp.id = 9;
				inGameHelp.msg = ItemConfig.SHOP_HAMMER_DESC + "\n" + ItemConfig.SHOP_HAMMER_PRICE + " gold." + "\n press \"SPACE\" to buy"; 
				...
				item.type =  ItemConfig.SHOP_HAMMER;
				item.desc =  ItemConfig.SHOP_HAMMER_DESC;
				item.price =  ItemConfig.SHOP_HAMMER_PRICE;
		...
		override public function onLevelStarted(e:CaveSmashEvent):void 
		{
			...
			if ( this != null ) {
				showItemStandInfo( ItemConfig.SHOP_HAMMER, gdc.hasComboAttack() );				
			}			
		}
