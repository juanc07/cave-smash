package components.interactive 
{
	 ...
	 * @author jc
	 */
	public class ItemStandShoes extends StaticItemBase
	{
		
		private var isHitPlayer:Boolean = false;
		private var inGameHelp:InGameMessageMC;
		
		public function ItemStandShoes() 
		{
			super();
		}
		...
		{
			if( !isHitPlayer &&  !gdc.hasDoubleJump() ){				
				isHitPlayer = true;
				inGameHelp = new InGameMessageMC();
				inGameHelp.id = 8;
				inGameHelp.msg = ItemConfig.SHOP_SHOES_DESC + "\n" + ItemConfig.SHOP_SHOES_PRICE + " gold." + "\n press \"SPACE\" to buy";
				...
				item.type =  ItemConfig.SHOP_SHOES;
				item.desc =  ItemConfig.SHOP_SHOES_DESC;
				item.price =  ItemConfig.SHOP_SHOES_PRICE;
		...
		override public function onLevelStarted(e:CaveSmashEvent):void 
		{
			...
			if ( this != null ) {
				showItemStandInfo( ItemConfig.SHOP_SHOES, gdc.hasDoubleJump() );
			}			
		}
