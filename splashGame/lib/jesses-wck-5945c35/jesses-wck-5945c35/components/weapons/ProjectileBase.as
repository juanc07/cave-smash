package components.weapons 
{
	 ...
	 * @author jc
	 */
	public class ProjectileBase extends BodyShape {	
		...
		public var isBossDarkRock:Boolean;
			...
			if( this != null && !hasRemoved && !isLevelComplete ){
				...
				for ( i = 0; i < blocksLen; i++){
					if (  e.other.GetBody().GetUserData() is CSObjects.MONSTER_BLOCKS[ i ] ) {
						if(!isBossDarkRock){
							trace("ProjectileBase hit monsterBlock " + CSObjects.MONSTER_BLOCKS[ i ]);
							removeME();						
							return;
						}
					}					
				...
				if (e.other.GetBody().m_userData is DarkBossMC ) {
					removeME();
					if (!isEnemyBullet && isBossDarkRock ) {
						boss = e.other.GetBody().m_userData;						
						TweenLite.delayedCall( HIT_EFFECT_DELAY, damageBoss  );
						return;
					}
		...
		private function damageBoss():void {
			...
			boss.hit(  currentDirection, true, 3 );
		}
